...
def test_radiator_area_smaller_for_colder_sink():
    # A colder sink (PSR) needs less radiator area for the same load.
    warm = radiator_area_m2(12.0, 330.0, 300.0, 0.9)
    cold = radiator_area_m2(12.0, 330.0, 70.0, 0.9)
    assert cold < warm
...
def test_radiator_benefit_scales_with_compute():
    # The siting benefit (radiator saved) is far larger per MW than at the small reference.
    r = estimate(n=2000)
    assert r.radiator_saved_t_per_mw > 10 * r.radiator_saved_t_at_ref
