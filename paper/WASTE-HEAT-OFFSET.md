...
equivalent displaced; route totals from the current `lpem` model, v0.10):

| Route | Offsettable (kWh/kg O2) | % of route total |
|---|---|---|
| **PSR water mining** | **2.08** | **14.8%** |
| H2 reduction (ilmenite) | 1.33 | 5.5% |
| Carbothermal (CH4) | 0.13 | 0.6% |
| Molten regolith electrolysis | 0.13 | 0.5% |
| Molten-salt (FFC) | 0.09 | 0.4% |

The water route is the clear beneficiary: its **entire** thermal-mining demand is
low-grade and so fully offsettable, ~15% of its total energy. The high-temperature routes
gain almost nothing (the share is now even smaller than in earlier model versions, because
their totals rose once continuous reactor heat loss was charged; the low-grade preheat
slice is a sliver of a larger denominator, and the energy that matters, the 1073-1900 K
reactor heat, is untouchable by a 350 K source).
